...
if (!empty($type)){
    ...
    $checkProdType_rs = Database::search("SELECT * FROM color WHERE `color`='".$type."'");
    $checkProdType_num = $checkProdType_rs->num_rows;

    if($checkProdType_num==0){
        Database::iud("INSERT INTO `color` (`color`) VALUES ('".$type."');");
        $getProdType_rs = Database::search("SELECT * FROM color");
